...
def get_w(frame_num, is_cyc, device):
    if frame_num < 5:
        if frame_num == 4:
            w_cyc = torch.tensor([[0.70, 0.10, 0.10, 0.10],
                                  [0.10, 0.70, 0.10, 0.10],
                                  [0.10, 0.10, 0.70, 0.10],
                                  [0.10, 0.10, 0.10, 0.70]
                                  ]).to(device)
            w_uncyc = torch.tensor([[0.70, 0.20, 0.10, 0.00],
                                    [0.15, 0.60, 0.15, 0.10],
                                    [0.10, 0.15, 0.60, 0.15],
                                    [0.00, 0.10, 0.20, 0.70]
                                    ]).to(device)
        elif frame_num == 3:
            w_cyc = torch.tensor([[0.70, 0.15, 0.15],
                                  [0.15, 0.70, 0.15],
                                  [0.15, 0.15, 0.70]
                                  ]).to(device)
            w_uncyc = torch.tensor([[0.70, 0.20, 0.10],
                                    [0.20, 0.60, 0.20],
                                    [0.10, 0.20, 0.70]
                                    ]).to(device)
        elif frame_num == 2:
            x = 0.25
            w_cyc = torch.tensor([[1 - x, x],
                                  [x, 1 - x],
                                  ]).to(device)
            w_uncyc = w_cyc
        elif frame_num == 1:
            w_cyc = torch.tensor([[1]]).to(device)
            w_uncyc = w_cyc
        else:
            raise NotImplementedError

        w = w_cyc if is_cyc else w_uncyc
        return w
    ...
    if frame_num < 13:
        ...
        v = [1, 1, 1, 1, 1, 2, 14, 2, 1, 1, 1, 1, 1]

    n = frame_num
    if is_cyc:
        cycle = np.array(v)
        extended_cycle = np.zeros(n)
        extended_cycle[:len(cycle)] = cycle
        max_index = np.argmax(extended_cycle)

        w = np.zeros((n, n))
        for i in range(n):
            shift = i - max_index
            w[i] = np.roll(extended_cycle, shift)
    else:
        w = np.zeros((n, n))
        mid = len(v) // 2
        np.fill_diagonal(w, v[mid])
        for i in range(1, mid + 1):
            np.fill_diagonal(w[i:], v[mid + i])
            np.fill_diagonal(w[:, i:], v[mid - i])
...
class UNetWrapper(nn.Module):
    ...
    def predict_with_decomposed_unconditional_scales(self, x, t, clip_embed, volume_feats, x_concat,
                                                     unconditional_scales):
        x_ = torch.cat([x] * 3, 0)
        t_ = torch.cat([t] * 3, 0)
...
class SpatialVolumeNet(nn.Module):
    def __init__(self, time_dim, view_dim, view_num,
        ...
        self.spatial_volume_feats = SpatialTime3DNet(input_dim=16 * view_num, time_dim=time_dim,
                                                     dims=(64, 128, 256, 512))
    ...
    def construct_spatial_volume(self, x, t_embed, v_embed, target_poses, target_Ks):
        ...
        spatial_volume_verts = torch.linspace(-self.spatial_volume_length, self.spatial_volume_length, V,
    ...
    def construct_view_frustum_volume(self, spatial_volume, t_embed, v_embed, poses, Ks, target_indices):
        ...
        near = torch.ones(B * TN, 1, H, W, dtype=spatial_volume.dtype,
                          device=spatial_volume.device) * self.default_origin_depth - self.frustum_volume_length
        far = torch.ones(B * TN, 1, H, W, dtype=spatial_volume.dtype,
                         device=spatial_volume.device) * self.default_origin_depth + self.frustum_volume_length

        target_indices = target_indices.view(B * TN)  # B*TN
        poses_ = poses[target_indices]  # B*TN,3,4
        Ks_ = Ks[target_indices]  # B*TN,3,4
        volume_xyz, volume_depth = create_target_volume(D, self.frustum_volume_size, self.input_image_size, poses_, Ks_,
        ...
        volume_feats = F.grid_sample(spatial_volume_, volume_xyz_, mode='bilinear', padding_mode='zeros',
...
class SyncMultiviewDiffusion(pl.LightningModule):
    ...
    def training_step(self, batch):
        ...
        clip_embed, volume_feats, x_concat = self.get_target_view_feats(input_info['x'], spatial_volume, clip_embed,
        ...
        self.log('sim', loss_simple.mean(), prog_bar=True, logger=True, on_step=True, on_epoch=True,
                 rank_zero_only=True)
    ...
    def add_noise(self, x_start, t):
        ...
        sqrt_one_minus_alphas_cumprod_ = sqrt_one_minus_alphas_cumprod_.view(B, *[1 for _ in
                                                                                  range(len(x_start.shape) - 1)])
        x_noisy = sqrt_alphas_cumprod_ * x_start + sqrt_one_minus_alphas_cumprod_ * noise
        return x_noisy, noise

    def sample(self, sampler, batch, cfg_scale, batch_view_num, return_inter_results=False, inter_interval=50,
               inter_view_interval=2, smooth_filter=True, is_cyc=False):
        _, clip_embed, input_info = self.prepare(batch)
        x_sample, inter = sampler.sample(input_info, clip_embed, unconditional_scale=cfg_scale,
                                         log_every_t=inter_interval, batch_view_num=batch_view_num,
                                         smooth_filter=smooth_filter, is_cyc=is_cyc)
    ...
    def log_image(self, x_sample, batch, step, output_dir):
        ...
        image_cond = []
        for bi in range(B):
            img_pr_ = concat_images_list(process(batch['input_image'][bi]),
                                         *[process(x_sample[bi, ni].permute(1, 2, 0)) for ni in range(N)])
...
class SyncDDIMSampler:
    def __init__(self, model: SyncMultiviewDiffusion, ddim_num_steps, ddim_discretize="uniform", ddim_eta=1.0,
                 latent_size=32):
        super().__init__()
        self.model = model
    ...
    def _make_schedule(self, ddim_num_steps, ddim_discretize="uniform", ddim_eta=0., verbose=True):
        self.ddim_timesteps = make_ddim_timesteps(ddim_discr_method=ddim_discretize, num_ddim_timesteps=ddim_num_steps,
                                                  num_ddpm_timesteps=self.ddpm_num_timesteps, verbose=verbose)  # DT
    ...
    def denoise_apply(self, x_target_noisy, input_info, clip_embed, time_steps, index, unconditional_scale,
                      batch_view_num=1, is_step0=False, smooth_filter=True, is_cyc=False):
        """
        @param x_target_noisy:   B,N,4,H,W
        @param input_info:
        @param clip_embed:       B,M,768
        ...
        spatial_volume = self.model.spatial_volume.construct_spatial_volume(
            x_target_noisy, t_embed, v_embed,
            self.model.poses, self.model.Ks)

        if smooth_filter:
            device = spatial_volume.device
            spatial_volume_ = torch.zeros_like(spatial_volume).to(device)
            frame_num = spatial_volume_.shape[0]
            w = get_w(frame_num, is_cyc, device)  # may not be normalized
            for i in range(frame_num):
                # TODO: schedule
                # t = time_steps[0]
                # w[i] = w[i] * (1 + 9 * (1000 - t) / 1000)
                spatial_volume_[i] = (spatial_volume * w[i][:, None, None, None, None]).sum(0) / w[i].sum()
            spatial_volume = spatial_volume_

        target_indices = torch.arange(N)  # N
        num_gpu = torch.cuda.device_count()
        batch_frame_num = B // num_gpu

        model_list = [self.model]
        for i in range(1, num_gpu):
            copied_model = deepcopy(self.model).to(torch.device(f'cuda:{i}'))
            model_list.append(copied_model)

        _ = torch.inverse(torch.ones((0, 0), device="cuda:0"))
        with ThreadPoolExecutor(max_workers=8) as executor:
            # for fi in range(0, B, batch_frame_num):
            e_t = []
            for ni in range(0, N, batch_view_num):  # N=batch_view_num=16
                futures = []
                for gpu_id in range(num_gpu):
                    fi = gpu_id * batch_frame_num
                    Bi = list(range(fi, fi + batch_frame_num))
                    Ni = list(range(ni, ni + batch_view_num))
                    B_ = len(Bi)

                    x_target_noisy_ = x_target_noisy[Bi, ni:ni + batch_view_num]
                    VN = x_target_noisy_.shape[1]
                    x_target_noisy_ = x_target_noisy_.reshape(B_ * VN, C, H, W)

                    time_steps_ = repeat_to_batch(time_steps[Bi], B_, VN)
                    target_indices_ = target_indices[ni:ni + batch_view_num].unsqueeze(0).repeat(B_, 1)

                    futures.append(
                        executor.submit(
                            self.pred_noise_batch,
                            model_list[gpu_id],
                            x_input[Bi], spatial_volume[Bi],
                            clip_embed[Bi], t_embed[Bi], v_embed[Bi], target_indices_,
                            x_target_noisy_, time_steps_, unconditional_scale, (Bi, Ni)
                        )
                    )

                noises = []
                for future in futures:
                    noise, index_ = future.result()
                    Bi, Ni = index_
                    noises.append(noise.view(B_, VN, 4, H, W).to(device=x_target_noisy.device))
    ...
    @torch.no_grad()
    def pred_noise_batch(self, cur_model, x_input, spatial_volume,
                         clip_embed, t_embed, v_embed, target_indices_,
                         x_target_noisy_, time_steps_, unconditional_scale,
                         index):

        with torch.autocast(device_type='cuda'):
            cur_device = cur_model.device
            x_input = x_input.to(cur_device)
            spatial_volume = spatial_volume.to(cur_device)
            clip_embed = clip_embed.to(cur_device)
            t_embed = t_embed.to(cur_device)
            v_embed = v_embed.to(cur_device)
            target_indices_ = target_indices_.to(cur_device)
            x_target_noisy_ = x_target_noisy_.to(cur_device)
            time_steps_ = time_steps_.to(cur_device)

            clip_embed_, volume_feats_, x_concat_ = cur_model.get_target_view_feats(
                x_input, spatial_volume,
                clip_embed, t_embed, v_embed,
                target_indices_
            )

            if type(unconditional_scale) == float:  ## CFG
                if unconditional_scale != 1.0:
                    noise = cur_model.model.predict_with_unconditional_scale(
                        x_target_noisy_, time_steps_, clip_embed_,
                        volume_feats_, x_concat_, unconditional_scale
                    )
                else:
                    noise = cur_model.model(
                        x_target_noisy_, time_steps_, clip_embed_,
                        volume_feats_, x_concat_, is_train=False
                    )
            else:  ## DG
                noise = cur_model.model.predict_with_decomposed_unconditional_scales(
                    x_target_noisy_, time_steps_, clip_embed_,
                    volume_feats_, x_concat_, unconditional_scale
                )
    ...
    @torch.no_grad()
    def sample(self, input_info, clip_embed,
               unconditional_scale=1.0, log_every_t=50, batch_view_num=1,
               smooth_filter=True, is_cyc=False):
        """
        ...
        x_target_noisy = torch.randn([1, N, C, H, W], device=device).expand([B, N, C, H, W])
        ...
        for i, step in enumerate(iterator):
            ...
            x_target_noisy = self.denoise_apply(
                x_target_noisy, input_info, clip_embed, time_steps, index, unconditional_scale,
                batch_view_num=batch_view_num, is_step0=index == 0,
                smooth_filter=smooth_filter, is_cyc=is_cyc)
