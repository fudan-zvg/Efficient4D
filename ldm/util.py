...
def prepare_inputs(image_path, elevation_input, crop_size=-1, image_size=256, frame_num=16):

    if '.png' in image_path or '.jpg' in image_path:
        images = [image_path]
    else:
        images = os.listdir(image_path)
        if 'frame' in images[0]:
            images.sort()
        else:
    ...
    for image in images:
        ...
        images_input.append(image_input.unsqueeze(0))

    images_input = torch.cat(images_input, dim=0)

    elevation_input = torch.from_numpy(np.asarray([np.deg2rad(elevation_input)], np.float32))
    elevation_input = torch.repeat_interleave(elevation_input.unsqueeze(0), num_imgs, dim=0)

    return {"input_image": images_input, "input_elevation": elevation_input}